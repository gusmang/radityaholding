...
<nav>
    <ul class="pagination">
        ...
        @if ($page >= $start && $page <= $end) @if ($page==$paginator->currentPage())
            <li class="page-item" aria-current="page" onClick="fetchPaginator('<?php echo $url; ?>')" style="cursor:pointer;" id="paginglink<?php echo $page; ?>"> <span class="page-link">{{ $page }}</span></li>
            @else
            <li class="page-item" id="paginglink<?php echo $page; ?>"><a class="page-link" onClick="fetchPaginator('<?php echo $url; ?>')" href="#">{{ $page }}</a></li>
