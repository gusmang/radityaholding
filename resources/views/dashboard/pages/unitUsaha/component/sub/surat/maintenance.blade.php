<div class="div_display_unit_sub" id="table_surat_maintenance" style="display: none;">
    <table class="table stripe hover nowrap">
        ...
        <tbody>
            ...
            $an = 0;
            @endphp
            @foreach($users_maintenance as $row)
            @php
            $an++;
            @endphp
            <tr>
                <td>
                    <input type="hidden" id={{ "id_role_pengadaan_maintenance_".$an }} name={{ "id_role_pengadaan_maintenance_".$an }}
                        class="form-control" value="{{ $row->id }}" style="width: 70px!important;" />
                    <input type="number" id={{ "role_pengadaan_maintenance_".$an }} name={{ "role_pengadaan_maintenance_".$an }}
                    ...
                    <label class="switch">
                        ...
                        ?>
                            <input type="checkbox" class="switch-input" value="1" checked
                                id={{ "checked_role_maintenance_".$an }} name={{ "checked_role_maintenance_".$an }}>
                        <?php
                        } else {
                        ?>
                            <input type="checkbox" class="switch-input" value="1" id={{ "checked_role_maintenance_".$an }}
                                name={{ "checked_role_maintenance_".$an }}>
                        <?php
                        }
                        ?>
                ...
                <td>
                    <select class="form-control" id={{ "scmaintenance_role_pengadaan_maintenance_".$an }}
                        name={{ "scmaintenance_role_pengadaan_maintenance_".$an }}>
